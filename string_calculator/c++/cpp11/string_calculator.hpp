...
namespace StringCalculator
{
  ...
  auto make_ints_from_str(std::string const& numsAsString, std::string const& sep, IterOut out) -> void
  {
    ...
    transform(sepNumsAsString, std::begin(ints), [](std::string const& str){ return std::stoi(str); });

    auto checkForNegNums = [](int i){ return i < 0; };
