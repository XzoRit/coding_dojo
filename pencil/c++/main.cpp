...
{
    durability() = default;
    explicit durability(int d) : value_{d}
    {
    }
...
struct length
{
    explicit length(int l) : value_{l}
    {
    }
    explicit operator bool() const
    {
...
{
    eraser() = default;
    explicit eraser(durability d) : durability_{d}
    {
    }
    template <class Range> Range erase(Range r)
    {
        std::replace_if(
            std::begin(r), std::end(r), [this](auto c) { return durability_.degrade(c); }, ' ');
        return r;
    }
...
struct pen
{
    pen(durability d, length l, eraser e) : initial_durability_{d}, durability_{d}, length_{l}, eraser_{e}
    {
    }
    template <class OutIter> void write_to(const std::string& txt, OutIter it)
    {
        for (auto c : txt)
        {
    ...
    template <class OutRange> void insert_to(const std::string& txt, OutRange out_range)
    {
    ...
    template <class Range> Range erase_from(Range range)
    {
...
struct scratch_pad
{
    scratch_pad(paper pap, pen p) : paper_{std::move(pap)}, pen_{std::move(p)}
    {
    }
    ...
    template <class Range> void insert(const std::string& txt, const Range& insert_range)
    {
...
TEST_CASE("As a pencil manufacturer "
          "I want writing to cause a pencil point to go dull "
          "so that I can sell more pencils")
{
    scratch_pad pad{paper{}, pen{durability{2}, length{1}, eraser{durability{4}}}};
    SUBCASE("lowercase letters should degrade the pencil point by a value of one")
    {
        pad.write("abc");
        CHECK(pad.text() == "ab ");
        SUBCASE("As a writer "
                ...
                "so that I can continue to write with it after it goes dull")
        {
            ...
            SUBCASE("When a pencil's length is zero, "
                    "then sharpening it "
    ...
    SUBCASE("capital letters should degrade the point by two")
    {
        pad.write("ABC");
        CHECK(pad.text() == "A  ");
    }
    SUBCASE("Writing spaces and newlines expends no graphite; "
          ...
          "so that I can remove my mistakes")
{
            ...
            "so that I can sell more pencils")
    {
                ...
                "should degrade the eraser by a value of one")
        {
            ...
            pad.erase("ab");
            CHECK(pad.text() == "  c  c");
            SUBCASE("Once the eraser durability is zero, "
